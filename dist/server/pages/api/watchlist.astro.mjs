...
const GET = async ({ url, request }) => {
  ...
  try {
    if (id) {
      ...
      const items = await query("SELECT * FROM watchlist_items WHERE watchlist_id=$1 ORDER BY added_at DESC", [id]);
...
const POST = async ({ request }) => {
  ...
  try {
    ...
    if (action === "create_list") {
      const [l] = await query("INSERT INTO watchlists(user_id,name) VALUES($1,$2) RETURNING *", [user.id, b.name || "New Watchlist"]);
      return json(l);
    }
    if (action === "add") {
      const [i] = await query("INSERT INTO watchlist_items(watchlist_id,symbol,notes) VALUES($1,$2,$3) ON CONFLICT(watchlist_id,symbol) DO UPDATE SET notes=EXCLUDED.notes RETURNING *", [b.watchlist_id, b.symbol?.toUpperCase(), b.notes || ""]);
      return json(i);
    }
    if (action === "remove") {
      await query("DELETE FROM watchlist_items WHERE id=$1", [b.item_id]);
      return json({ success: true });
    }
    if (action === "delete_list") {
      await query("DELETE FROM watchlists WHERE id=$1 AND user_id=$2", [b.id, user.id]);
      return json({ success: true });
    }
