import { a as fetchStockData } from './yahoo_r_Nj-wD_.mjs';
...
const screenerHandler = async ({ request }) => {
  const { filters = {} } = await request.json();
  const universe = filters.symbols?.length ? filters.symbols : UNIVERSE;
  const BATCH = 5;
  const results = [];
  ...
  let s = results;
  if (filters.rsiMin != null) s = s.filter((x) => x.rsi >= filters.rsiMin);
  if (filters.rsiMax != null) s = s.filter((x) => x.rsi <= filters.rsiMax);
  ...
  if (filters.changePctMin != null) s = s.filter((x) => x.changePct >= filters.changePctMin);
  if (filters.changePctMax != null) s = s.filter((x) => x.changePct <= filters.changePctMax);
  return json({ results: s, total: s.length });
};
const compareHandler = async ({ url }) => {
  ...
  return json({ symbols, data: results.map((r, i) => r.status === "fulfilled" ? r.value : { symbol: symbols[i], error: r.reason?.message }) });
};
const exportHandler = async ({ url }) => {
  ...
  const { quote, chartData } = await fetchStockData(symbol, url.searchParams.get("period") ?? "6mo", "1d");
...
export { compareHandler as c, exportHandler as e, screenerHandler as s };
